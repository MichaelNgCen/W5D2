...
def population_in_millions
  ...
  # millions.
  execute(<<-SQL)
  SELECT name, (population/1000000) FROM countries WHERE continent LIKE 'South America'
  SQL
end
...
def name_and_population
  ...
  execute(<<-SQL)
  SELECT name, population FROM countries WHERE name LIKE 'France' OR name LIKE 'Germany' OR name LIKE 'Italy'
  SQL
end
